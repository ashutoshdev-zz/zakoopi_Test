...
public class UserFeedAdapter extends BaseAdapter {
	...
	public UserFeedAdapter(Context context, List<UserFeedPojo> list,
						...
						try {
							...
							if (isInternetPresent) {
								...
								LookbookDialog(ctx, pojo.getIdd(), user_name,
										url_img, pojo.getTitle(),
										pojo.getHits(), pojo.getLikes(),
										pojo.getComment_count(),
										pojo.getIs_liked(), position);
						...
						try {
							...
							if (listApp != null) {
								...
								customDialog(holder.look_image, "Lookbooks",
										pojo.getSlug(), pojo.getTitle(),
										pojo.getIdd(), pro_user_id);
							}
	...
	private void share(ResolveInfo appInfo, final ImageView img,
			final String mode1, final String slug1, final String share_text1,
			final String mode_id, final String user_id) {
		...
		if (mode1.equals("StoreReviews")) {
			...
			String share_all_text = share_text1 + " | Zakoopi"
					+ System.getProperty("line.separator");
			share_all_text = share_all_text + "http://www.zakoopi.com/" + mode1
					+ "/" + slug1 + System.getProperty("line.separator")
					+ System.getProperty("line.separator");
			;
			...
			public void onClick(View v) {
				...
				if (listApp != null) {
					...
					customDialog(img_user_image, "StoreReviews",
							pojo.getSlug(), share_text, pojo.getIdd(),
							pro_user_id);
					// dd.cancel();
				}
	...
	private View setHeader(final Context ctxx, final String lookbook_id,
		...
		try {
			...
			Picasso.with(ctxx).load(pro_user_pic_url)
					.placeholder(R.drawable.profile_img_3).resize(50, 50)
					.into(user_pic);
