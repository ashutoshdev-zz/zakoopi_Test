...
public class StoreFeatureAdapter extends BaseAdapter {
	Context ctx;
	LayoutInflater inf;
	int displayWidth;
	Typeface typeface_semibold, typeface_black, typeface_bold, typeface_light,
			typeface_regular;
	List<StoreLookbookPojo> list;
...
Boolean isInternetPresent = false;
	...
	public StoreFeatureAdapter(Context ctxx, List<StoreLookbookPojo> list,
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		case 0:
			...
			if (result == null) {
				...
				holder.like_image.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						StoreLookbookPojo ppp = (StoreLookbookPojo) img
								.getTag();
						String string_like;
						String post_mode = "Lookbooks";
						String article_iddd = ppp.getMode_id();
						String get_likes = ppp.getLikes_count();

						if (ppp.getIs_liked().equals("true")) {

							holder.like_image
									.setImageResource(R.drawable.home_like_inactive);
							long count = Long.parseLong(ppp.getLikes_count()) - 1;
							holder.lookbook_like.setText("" + count);
							ppp.setIs_liked("false");
							ppp.setLikes_count("" + count);
							string_like = "0";
							...
							t.send(new HitBuilders.EventBuilder()
									.setCategory(
											"Unlike Lookbook on Store Feature")
									.setAction(
											"Unliked Lookbook by "
													+ pro_user_name)
									.setLabel("Store Feature").build());
							...
							t.send(new HitBuilders.EventBuilder()
									.setCategory(
											"Like Lookbook on Store Feature")
									.setAction(
											"Liked Lookbook by "
													+ pro_user_name)
									.setLabel("Store Feature").build());
			...
			final StoreLookbookPojo pojo = list.get(position);
			...
			try {
				...
				if (pojo.getIs_liked().equals("true")) {
					...
					holder.like_image
							.setImageResource(R.drawable.home_like_active);
					holder.lookbook_like.setText(pojo.getLikes_count());
				} else {
				...
				int hits_count = Integer.parseInt(pojo.getHits_text());
				if (hits_count >= 100 ) {
						...
						isInternetPresent = cd.isConnectingToInternet();
			             // check for Internet status
			                if (isInternetPresent) {
								...
								t.send(new HitBuilders.EventBuilder()
										.setCategory("Profile view on Store Feature")
										.setAction(
								...
								try {
									...
									isInternetPresent = cd.isConnectingToInternet();
						             // check for Internet status
						                if (isInternetPresent) {
						                    // Internet Connection is Present
						                	LookbookDialog(ctx, pojo.getMode_id(),
											...
											t.send(new HitBuilders.EventBuilder()
													.setCategory(
															"Lookbook view on Store Feature")
													.setAction(
					...
					public void onClick(View v) {
						...
						if (listApp != null) {
							...
							customDialog(holder.look_image, "Lookbooks",
									pojo.getSlug(),pojo.getTitle(), pojo.getMode_id(), pro_user_id);
						}
						...
						t.send(new HitBuilders.EventBuilder()
								.setCategory("Share on Store Feature")
								.setAction(
										"Shared Lookbook by " + pro_user_name)
								.setLabel("Store Feature").build());
		...
		case 1:
						...
						.setOnClickListener(new OnClickListener() {
							...
							public void onClick(View v) {
								...
								if (ppp.getIs_liked().equals("true")) {
									...
									t.send(new HitBuilders.EventBuilder()
											.setCategory(
													"Unlike Article on Store Feature")
											.setAction(
													"Unliked Article by "
															+ pro_user_name)
											.setLabel("Store Feature").build());
			...
			final StoreLookbookPojo article_pojo = list.get(position);
			...
			try {
				...
				final int set_height = (int) (((width1 * 1.0) / Integer
						.parseInt(article_pojo.getMedium_img_h())) * height1);
						...
						try {
							...
							isInternetPresent = cd.isConnectingToInternet();
				             // check for Internet status
				                if (isInternetPresent) {
				                    // Internet Connection is Present
				                	ArticleDialog(ctx, article_pojo.getMode_id(),
											article_pojo.getUsername(), article_pojo.getUserimg(),
											article_pojo.getTitle(),
											article_pojo.getHits(),
											article_pojo.getLikes_count(),
											article_pojo.getDescription(),
											article_pojo.getIs_new(),
							...
							public void onClick(View v) {
								...
								if (listApp != null) {
									...
									customDialog(article_holder.look_image,
											"Articles", article_pojo.getSlug(),article_pojo.getTitle(), article_pojo.getMode_id(), pro_user_id);
								}
...
if (mode1.equals("StoreReviews")) {
	...
	public void LookbookDialog(Context ctxx, String lookbook_id,
		...
		txt_head.setTypeface(typeface_bold);
	...
	private View setHeader(final Context ctxx, final String lookbook_id,
		...
		img_like.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				StoreLookbookPojo pojo1 = list.get(pos);
				if (st.equals("true")) {
					...
					pojo1.setLikes_count(String.valueOf(Integer.parseInt(pojo1
							.getLikes_count()) - 1));
	...
	private View setHeaderArticle(final Context ctxx, final String article_id,
		...
		img_like.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				StoreLookbookPojo pojo2 = list.get(pos);
				if (statu.equals("true")) {
					...
					pojo2.setLikes_count(String.valueOf(Integer.parseInt(pojo2
							.getLikes_count()) - 1));
