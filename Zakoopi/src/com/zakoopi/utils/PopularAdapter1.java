...
public class PopularAdapter1 extends BaseAdapter {
	...
	public void addItemsNew(List<POJO> newItems) {
		if (null == newItems || newItems.size() <= 0) {
			return;
		}
	...
	public void addColorsNew(List<Integer> newItems) {
		if (null == newItems || newItems.size() <= 0) {
			return;
		}
		...
		try {
			...
			case 0:
				...
				if (result == null) {
					...
					result = inf.inflate(R.layout.lookbook_feed_item, parent,
							false);
						...
						public void onClick(View v) {
							...
							if (ppp.getis_liked().equals("true")) {
								...
								t.send(new HitBuilders.EventBuilder()
										.setCategory(
												"Unlike Lookbook on Featured Feed")
										.setAction(
												"Unliked Lookbook by "
														+ pro_user_name)
								...
								t.send(new HitBuilders.EventBuilder()
										.setCategory(
												"Like Lookbook on Featured Feed")
										.setAction(
												"Liked Lookbook by "
														+ pro_user_name)
										.setLabel("Featured Feed").build());
				...
				try {
					...
					Picasso.with(ctx).load(pojo.getUserimg())
							...
							.resize(50, 50).into(holder.user_image);
					...
					int hits_count = Integer.parseInt(pojo.getHits_count());
					if (hits_count >= 100) {
						...
						public void onClick(View v) {
							...
							if (isInternetPresent) {
								...
								if (pojo.getUserid().equals(pro_user_id)) {
									...
									// Variables.user_id_list.clear();
									Intent user = new Intent(ctx,
					...
					holder.title.setOnClickListener(new View.OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							if (isInternetPresent) {
								// Internet Connection is Present
								LookbookDialog(ctx, pojo.getIdd(),
										pojo.getUsername(), pojo.getUserimg(),
							...
							.setOnClickListener(new OnClickListener() {
								...
								public void onClick(View v) {
									...
									if (listApp != null) {
										...
										customDialog(holder.look_image,
												"Lookbooks", pojo.getSlug(),
												pojo.getTitle(), pojo.getIdd(),
												pro_user_id);
									}
			...
			case 1:
							...
							.setOnClickListener(new OnClickListener() {
								...
								public void onClick(View v) {
									...
									if (ppp.getis_liked().equals("true")) {
										...
										t.send(new HitBuilders.EventBuilder()
												.setCategory(
														"Unlike Article on Featured Feed")
												.setAction(
														"Unliked Article by "
																+ pro_user_name)
												.setLabel("Featured Feed")
				...
				try {
					...
					article_holder.user_name
							.setText(article_pojo.getUsername());

					Picasso.with(ctx).load(article_pojo.getUserimg())
							.placeholder(R.drawable.profile_img_3)
							.resize(50, 50).into(article_holder.user_image);
					...
					int hits_count = Integer.parseInt(article_pojo
							.getHits_count());
					if (hits_count >= 100) {
						article_holder.lookbook_view
								.setVisibility(View.VISIBLE);
						article_holder.lookbook_view.setText(article_pojo
								.getHits());
							...
							.setOnClickListener(new OnClickListener() {
								...
								public void onClick(View v) {
									...
									if (isInternetPresent) {
										...
										Intent user = new Intent(ctx,
												OtherUserProfile.class);
										user.putExtra("user_id",
												article_pojo.getUserid());
										ctx.startActivity(user);
			...
			case 2:
				...
				if (review_result == null) {
					LayoutInflater inf = (LayoutInflater) ctx
							.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
					review_result = inf.inflate(R.layout.review_feed_item,
							parent, false);
							...
							.setOnClickListener(new OnClickListener() {
								...
								public void onClick(View v) {
									...
									if (ppp.getis_liked().equals("true")) {
										...
										t.send(new HitBuilders.EventBuilder()
												.setCategory(
														"Unlike Review on Featured Feed")
												.setAction(
														"Unliked Review by "
																+ pro_user_name)
												.setLabel("Featured Feed")
				...
				try {
					...
					Picasso.with(ctx).load(review_pojo.getUserimg())
							.placeholder(R.drawable.profile_img_3)
							.resize(50, 50).into(review_holder.user_image);
					...
					int hits_count = Integer.parseInt(review_pojo
							.getHits_count());
					if (hits_count >= 100) {
						review_holder.lookbook_view.setVisibility(View.VISIBLE);
						review_holder.lookbook_view.setText(review_pojo
								.getHits());
					...
					String review_data = String.valueOf(Html
							.fromHtml(review_pojo.getReview()));
					...
					if (review_data.length() > 400) {
						...
						review_holder.review.setText(Html
								.fromHtml(upToNCharacters + more));
					} else {
						review_holder.review.setText(Html.fromHtml(review_pojo
								.getReview()));
					}

					review_holder.store_name.setText(review_pojo
							.getStore_name());
					...
					if (review_pojo.getRated().equals("0")) {
						...
						review_holder.store_rate
								.setText(review_pojo.getRated());
						drawable.setColor(Color.parseColor(review_pojo
								.getRated_color()));
					}
			...
			case 3:
				...
				if (team_result == null) {
					LayoutInflater inf = (LayoutInflater) ctx
							.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
					team_result = inf.inflate(R.layout.team_feed_item, null,
							false);
				...
				try {
					 ...
					 * (), team_holder.look_image, options,
					 * animateFirstListener);
					 */
	...
	public void customDialog(final ImageView img, final String mode,
			final String slug, final String share_text, final String mode_id,
		...
		ggg.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				share(listApp.get(position), img, mode, slug, share_text,
						mode_id, user_id);
				dd.cancel();
			}
		});
		...
		share_image.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (listApp != null) {
					...
					customDialog(img_user_image, "StoreReviews",
							pojo.getSlug(), share_text, pojo.getIdd(), pojo.getUserid());
					// dd.cancel();
				}
			...
			public void onClick(View v) {
				...
				try {
					...
					if (pojo.getis_liked().equals("true")) {
						...
						t.send(new HitBuilders.EventBuilder()
								.setCategory(
										"Unlike Review on Featured Feed Full Review")
						...
						t.send(new HitBuilders.EventBuilder()
								.setCategory(
										"Like Review on Featured Feed Full Review")
				...
				if (isInternetPresent) {
					...
					try {
						...
						if (!edt_comment.getText().toString().equals("")) {
							...
							addOwnComment(mode_id, edt_comment.getText()
									.toString(), "LBK", comment_count);
					...
					/*Toast.makeText(ctx, "Please connect to internet",
	...
	//	Log.e("Article_id", article_id + " " + article_title);
		typeface_semibold = Typeface.createFromAsset(ctx.getAssets(),
				"fonts/SourceSansPro-Semibold.ttf");
				...
				if (isInternetPresent) {
					...
					try {
						...
						if (!edt_comment.getText().toString().equals("")) {
							...
							edt_comment.setText("");
							edt_comment.setHint("Add a comment....");
							addOwnCommentArticle(article_id, edt_comment
									.getText().toString(), "ART", comment_count);

							/*
							 * Tracker t = ((UILApplication) getApplication())
							 * .getTracker(TrackerName.APP_TRACKER); // Build
							 * and send an Event.
							 * 
							 * t.send(new HitBuilders.EventBuilder()
